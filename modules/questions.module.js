...
const findquestion_textt=(question_text,callback)=>{
    const query='select * from questions where question_text COLLATE utf8mb4_unicode_ci like ?';
    db.query(query,[`%${question_text}%`],(err,result)=>{
        if(err)
        {
            return callback(err,null);
        }
...
module.exports={getallquestions,getallquestionsbyId,addquesiton,updatequesiton,deletequesiton,findquestion_textt};