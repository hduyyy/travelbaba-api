...
const getallshopping_centers=(req,res)=>{
    shopping_centersmodule.getallshopping_centers((err,result)=>{
        ...
        const dataFormat=result.map(e=>{
            const shopping_center_image_base64=e.shopping_center_image?e.shopping_center_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            const content_image_base64=e.content_image?e.content_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            return{
                e,shopping_center_image_base64,content_image_base64
            }
        })
...
const getallshopping_centerssbyId=(req,res)=>{
    shopping_centersmodule.getallshopping_centersbyId(req.params.shopping_center_id,(err,result)=>{
        ...
        const shopping_center_image_base64=result.shopping_center_image?result.shopping_center_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
        ...
        return res.status(201).json({code:201,message:'Successfull',result,shopping_center_image_base64,content_image_base64});
    });
};
