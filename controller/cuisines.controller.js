...
const getallcuisines=(req,res)=>{
    cuisinesmodule.getallcuisines((err,result)=>{
        ...
        const dataFormat=result.map(e=>{
            const cuisines_image_base64=e.cuisines_image?e.cuisines_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            const content_image_base64=e.content_image?e.content_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            return{
                e,cuisines_image_base64,content_image_base64
            }
        })
...
const getallcuisinesbyId=(req,res)=>{
    cuisinesmodule.getallcuisinesbyId(req.params.cuisines_id,(err,result)=>{
        ...
        const cuisines_image_base64=result.cuisines_image?result.cuisines_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
        ...
        return res.status(201).json({code:201,message:'Successfull',result,cuisines_image_base64,content_image_base64});
        });
};
