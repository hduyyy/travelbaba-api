...
const getallevents=(req,res)=>{
    eventsmodule.getallevents((err,result)=>{
        ...
        const dataFormat=result.map(e=>{
            const eventsImages = e.event_image ? e.event_image.toString('utf-8').split(',').map(img => {
                const filename = path.basename(img); 
                return `http://localhost:3001/upload/images/${filename}`; 
            }) : [];
            const contentImages = e.content_image ? e.content_image.toString('utf-8').split(',').map(img => {
                const filename = path.basename(img);
                return `http://localhost:3001/upload/images/${filename}`; 
            }) : [];
            console.log('eventsImages',eventsImages);
            console.log('contentImages',contentImages);
             const event_image_base64 =e.event_image? e.event_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            const content_images_base64 =e.content_image? e.content_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
     console.log(event_image_base64);
    console.log(content_images_base64);
    return{
        e,eventimg_url:event_image_base64,contentimg_url:content_images_base64
    }          
        });

            return res.status(201).json({code:201,message:'Successfull',data:{
               dataFormat
            }});
    });
};
const getalleventsbyId=(req,res)=>{
    eventsmodule.getalleventsbyId(req.params.event_id,(err,result)=>{
        ...
        if(!result)
        {
            ...
            const event_image_base64=result.event_image?result.event_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            const content_image_base64=result.content_image?result.content_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
        return res.status(201).json({code:201,message:'Successfull',data:result,event_image_base64,content_image_base64})
    });
};
...
const getalleventsimgbyId = (req, res) => {
    eventsmodule.getalleventimagesbyid(req.params.event_id, (err, result) => {
        ...
        console.log('eventsImages',eventsImages);
        console.log('contentImages',contentImages);
         const event_image_base64 =result.event_image? result.event_image.toString('base64'):[];
        const content_images_base64 =result.content_image? result.content_image.toString('base64'):[];
...
console.log(content_images_base64);
        ...
        res.status(200).json({
            ...
            content_images: content_images_base64,
            event_image_url:eventsImages,
            content_images_url:contentImages
        });
    });
};
