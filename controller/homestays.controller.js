...
const Findhomestaytitle=(req,res)=>{
    const {title}=req.body;
    if(!title)
    {
        return res.status(404).json({code:404, message: "error while input homestays title" });
    }
    homestaysmodule.findhomestaytitle(title,(err,result)=>{
        if(err)
        {
            return res.status(500).json({code:500, message: "Error find homestays title", error:err });
        }
        return res.status(201).json({code:201,message:'Find success', data:result});
    });
};
module.exports={getallhomestays,getallhomestaysbyId,addhomestays,updatehomestays,deletehomestays,Findhomestaytitle};