...
const getallmoves=(req,res)=>{
    movesmodule.getallmoves((err,result)=>{
        ...
        const dataFormat=result.map(e=>{
            const moves_image_base64=e.moves_image?e.moves_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            const content_image_base64=e.content_image?e.content_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
            return{
                e,moves_image_base64,content_image_base64
            }
        })
...
const getallmovesbyId=(req,res)=>{
    movesmodule.getallmovesbyId(req.params.moves_id,(err,result)=>{
        ...
        const moves_image_base64=result.moves_image?result.moves_image.toString('utf8').split(',').map(img=>Buffer.from(img).toString('base64')):[];
        ...
        return res.status(201).json({code:201,message:'Successfull',result,moves_image_base64,content_image_base64});
    });
};
